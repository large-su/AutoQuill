...
# ============================================================
# 题材分类（看板/反馈闭环共用，单一事实源）
# 规则顺序即优先级：标题+正文开头命中即归入，未命中为「其他」。
# 原定义位于 webui/published.py，2026-08 下沉以被 core/feedback_loop
# 与 workflows 选题打分复用（前端 /api/dashboard 语义保持不变）。
# ============================================================
...
def classify_genre(text):
    ...
    text 传标题与正文拼成的字符串；检测对文本长度不敏感（内部截断
    前 120 字符，与看板历史规则一致）。
    """
