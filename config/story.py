...
__all__ = [
    ...
    # 选题先验（2026-09-23 复盘：题型/降权/题材先验观测口径）
    "TOPIC_TYPE_PRIOR_ENABLE", "TOPIC_TYPE_PENALTY",
    "STORY_DOWNWEIGHT_KEYWORDS", "STORY_DOWNWEIGHT_FACTOR",
...
# 这些是「纯书单/写作教学/变现」类——答案是清单或方法论，不是故事体，
# 曾导致「通过了硬性筛选却写不出好故事」的痛点。
...
# 降权关键词（2026-09-23 复盘，从 STORY_EXCLUDE_KEYWORDS 挪出来）：
# 「求推荐 / 小说推荐 / 好看的小说」这类题的读者确实可能想要书单，但它们的
# **流量池大得多**，一刀切排除的代价太高——回放 46 篇有反馈的稿件，被这条
# 黑名单拦掉的 5 篇里有本账号本期第一（3639 阅读 / 33 赞 / 14 藏，标题命中
# 「小说推荐」）。改成命中即打分打折：仍可被选中，只是排在其它题后面；
# 真正纯找书（书荒/书单/求书…）仍走上面的硬排除。
STORY_DOWNWEIGHT_KEYWORDS = [
    # 注意「文推荐」是子串陷阱：它同时命中「爽文推荐」「虐文推荐」这类正常题，
    # 所以也放到降权而不是排除（2026-09-23 回放时发现的）。
    "求推荐", "小说推荐", "推荐小说", "好看的小说", "文推荐", "推荐几本",
    "有哪些推荐",
]

# 降权系数（0.6 = 打分乘 0.6）；设 1.0 等于关闭降权。
STORY_DOWNWEIGHT_FACTOR = 0.6

# ── 题型先验（2026-09-23 复盘 P1）──
# 实测（46 篇有反馈稿件，阅读/天中位）：命题作文/微小说 **2.0**、求推荐 13.4、
# 观点/讨论 28.5 —— 题型之间的差距比任何正文特征都大。命题作文类
# （"以…为开头写一篇微小说""用十个字写一个故事"）流量池小，命中即打分乘
# TOPIC_TYPE_PENALTY。**是打折不是排除**：万一某屏全是这类题，仍会被选中，
# 不至于让选题空转报错。
TOPIC_TYPE_PRIOR_ENABLE = True
TOPIC_TYPE_PENALTY = 0.4
...
# 开篇起手方式（2026-09-19 用户口径）：不搞固定循环，优先模仿「本题最受认可
# 那篇参考文章」的起手方式（学手法、不抄句子）；没有参考文章时才用轮换表兜底。
OPENING_MIRROR_REFERENCE = True

# 「模仿起手方式」的抄袭红线：故事引言与参考回答开头的最长连续重合字数达到
# 该值即判抄（换词不换骨架的改写通常留 8-15 字连续片段；12 可避开
# 「我把离婚协议放在茶几上」这类通用短句）。命中后带反馈重写，仍命中则整轮弃稿。
OPENING_COPY_MIN_RUN = 12

# 开篇起手式多样化（防「篇篇都以『我』字开头」的同构）：
# 真实产物统计（2026-09-19）：经典模式最近 22 篇里 20 篇以「我」起手（最近 20 篇
# 100%），根因是「第一人称声口 + 引言一票否决 + 禁空镜开场 + 首句进动作」叠加
# 把开头挤成唯一解。开启后按篇轮换指定起手式（对话/他人/物件/时间/反差），
# 一次运行内不重样；关掉则退回到不注入（纯净模式本来就不注入）。
...
# 题材先验只采纳「发布满 N 天」的篇目（2026-09-23 复盘 P5）。
# 为什么：本期两篇爆款在发布第 3 天只有 674 / 296 阅读、9 / 2 个赞，若那时就
# 计入先验，会得出「先婚后爱/追妻火葬场是劣质题材」——而它们最终贡献了本期
# 45% 的赞同（61/135）。用前三天的数据调选题，等于只看首映三天票房就决定
# 「这个类型不能拍」，而且错误会自我强化（越不选越拿不到翻身数据）。
# 0 = 关闭该保护（恢复旧行为）。
TOPIC_GENRE_PRIOR_MIN_AGE_DAYS = 7
