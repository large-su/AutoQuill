...
- **修复二（按篇轮换注入）**：新增 `next_opening_style()` 进程内轮换（多线程安全），
  `build_story_prompt` 按篇注入「本篇指定的开篇起手式」，并把选择写进日志 mode_str
  （`采样模式… · 起手式:对话起手`）——各篇生成互相看不见，只有外部轮换能保证一次运行内
  不重样；纯净模式刻意不注入（它本来就多样）
- **开关**：`config/story.py` 的 `OPENING_VARIETY`（默认 True；关掉即退回原行为）
- 新增 tests/test_opening_variety.py（9 例：轮换覆盖与回环、多线程不重不漏、指定起手式
  覆盖轮换、开关关闭不注入、纯净模式不受影响、起手式自带格式底线与人物在场要求）
